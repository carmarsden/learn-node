    ...
    it('should generate an array of 5', () => {
        return request(app)
            ...
            .then(res => {
                ...
                expect(res.body).to.include(5); // include 5
                expect(res.body).to.include.members([1,2,3,4]); // include all listed
                expect(res.body).to.have.members([1,2,3,4]); // have these members and no others
                expect(res.body).to.be.an('array').that.have.members([1,2,3,4,5]); // above assertions chained
            });
    })
});